...
export const execute: Execute = async (bot: Bot, message: Message) => {
  ...
  if (!args[0] || isNaN(parseInt(args[0])) || parseInt(args[0]) < 1) {
    const response = await message.reply(
      `Debes utilizar ${bot.config.prefix}clear {Número} para utilizar el comando correctamente. (Ese numero no puede ser cero!)`
    );
  ...
  if (deleteCount > parseInt(bot.config.maxDeleting)) {
    await message.channel.send(
      `No puedo borrar más de ${bot.config.maxDeleting} mensajes.`
    );
    ...
    const response2 = await message.channel.send(
      `Borrados '${bot.config.maxDeleting}' mensajes!`
    );
